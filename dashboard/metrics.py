...
def branch_table(
    ...
    for row in current.itertuples():
        ...
        # 계산한다.
        growth = _to_float(getattr(row, "customer_growth_yoy", None))
        if growth is None:
            growth = yoy_rate(
        ...
        rows.append(
            {
                ...
                "male_share": _to_float(row.male_share),
                "average_age": _to_float(row.average_age),
                "recent_signup_share": _to_float(row.recent_signup_share),
                "recommendation_share": _to_float(row.recommendation_share),
                "grade_s_share": _to_float(row.grade_s_share),
            }
        )
    ...
    given_total = (
        _row_for_month(summary_total, current_month)
...
def _total_row_from_source(row: pd.Series) -> dict[str, float | str | None]:
    ...
    값을 다시 계산하지 않는다.
    """
    count = _to_float(row["customer_count"])
    ...
    for column in TABLE_COLUMNS:
        if column in ("branch_name", "customer_count"):
            continue
        total_row[column] = (
            _to_float(row[column]) if column in row.index else None
        )
    return total_row
