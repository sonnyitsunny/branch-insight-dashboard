...
from dashboard.data import DashboardData, TOTAL_LABEL
...
def build_scatter_figure(data: DashboardData):
    scatter = metrics.growth_scatter(data.monthly)
    return figures.create_growth_scatter_figure(scatter, metrics.median_customer_count(scatter))


def build_age_figure(data: DashboardData, branch_name: str):
    distribution = metrics.age_distribution(data.age, branch_name)
...
def build_investment_figure(data: DashboardData, scope: str = TOTAL_LABEL):
    breakdown = metrics.investment_breakdown(data.investment, scope)
