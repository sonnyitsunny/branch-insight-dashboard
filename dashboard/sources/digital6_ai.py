"""원본 파일 — 디지털 채널 탭의 AI 요약.

앞의 AI요약 파일들과 컬럼도 값 형식도 같다. 읽는 일은 `ai_summary`가
맡고, 여기에는 그 파일만의 것 — 파일 이름, 환경 변수, 어느 탭의 글인지 —
만 적는다(→ revenue2_ai.py와 같은 골격).

디지털채널1~4는 숫자를 담은 원본이라 `digital1.py`~`digital4.py`가 쓰고
있다. 이 AI 요약 원본은 다섯째 파일이 아니라 여섯째 파일 이름으로
온다(→ FILE). 원본 쪽 파일 이름 규칙이라 여기서 정하지 않는다.
"""
...
from dashboard.data import AI_TOPIC_DIGITAL
from dashboard.sources import ai_summary

# 실제 데이터를 붙일 때 여기만 고치면 된다.
#
# 파일 이름만 적으면 app.py 옆의 `data/` 폴더에서 찾는다.
#   FILE = "디지털채널6_AI요약.pkl"       → data/디지털채널6_AI요약.pkl
# 환경 변수를 지정하면 아래 값보다 환경 변수가 우선한다.
FILE = "디지털채널6_AI요약.pkl"
FILE_ENV = "DASHBOARD_DIGITAL6_AI_FILE"

LABEL = "디지털채널 AI요약"

# 어느 탭의 글인지 가르는 값(→ data.AI_TOPICS).
TOPIC = AI_TOPIC_DIGITAL
