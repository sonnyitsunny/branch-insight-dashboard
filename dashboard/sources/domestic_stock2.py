"""원본 파일 — 시가총액 상위 종목의 지점별 국내주식 거래.

시장 전체의 시가총액 상위 N종목을 먼저 고르고, 그 종목마다 지점별 거래를
담는다. 상품 국내주식1과 컬럼 이름·단위가 같지만 행을 고르는 기준이 다르다.
1은 지점마다 그 지점의 상위 종목을 순위와 함께 담고, 이쪽은 모든 지점이
같은 종목 목록을 놓고 본다. 그래서 순위 축이 없고 종목이 축이다
...
단위 — 시가총액은 **억원**, 거래대금과 순매수금액은 **원**이다. 원본이 담은
단위를 그대로 두고 여기서 환산하지 않는다.

부호 — 순매수금액은 순매도인 종목에서 음수가 되며 원본이 앞에 `-`를 붙여
담는다. 인원수와 달리 음수를 막지 않는다.

업종 — 원본에 비어 있는 행이 있다. 값을 지어내지 않고 빈 값 그대로 넘긴다.

같은 종목의 시가총액과 업종은 지점이 달라도 같아야 한다. 시가총액은 시장이
정하는 값이고 업종은 종목의 성질이기 때문이다. 지점마다 다르면 파일이 잘못
붙었다는 뜻이라 멈춘다(→ check_stock_facts).
...
#   FILE = "상품_국내주식2.pkl"     → data/상품_국내주식2.pkl
# 환경 변수를 지정하면 아래 값보다 환경 변수가 우선한다.
FILE = "상품_국내주식2.pkl"
FILE_ENV = "DASHBOARD_DOMESTIC_STOCK2_FILE"

LABEL = "상품 국내주식2"

# 원본 컬럼명 → 내부 표준 컬럼명.
# ITM_ABRV_NM(종목약명)이 종목명이다. 상품 국내주식1의 '종목명'과 같은 값을
# 가리키므로 표준 이름을 `stock_name` 하나로 맞춘다.
COLUMNS: dict[str, str] = {
    "기준월": "base_month",
    "CSMT_ORZ_CD": "branch_id",
    "CSMT_ORZ_NM": "branch_name",
    "ITM_ABRV_NM": "stock_name",
    "업종": "sector",
    "시가총액": "market_cap",
...
STOCK_FACT_COLUMNS = ("market_cap", "sector")
...
def build(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    stock["base_month"] = to_month_column(
        stock["base_month"], "domestic_stock2"
    )
...
def check_one_row_per_stock(stock: pd.DataFrame) -> None:
    ...
    """
    keys = ["base_month", "branch_id", "stock_name"]
    duplicated = stock.duplicated(subset=keys)
    if duplicated.any():
        row = stock.loc[duplicated].iloc[0]
        raise ValueError(
            f"{LABEL} 파일에 같은 종목이 두 번 이상 있습니다: "
...
def check_stock_facts(stock: pd.DataFrame) -> None:
    """한 종목의 시가총액·업종이 지점마다 같은지 본다.

    시가총액은 시장이 정하는 값이고 업종은 종목의 성질이라 지점에 따라
    달라질 수 없다. 다르다면 종목명이 겹쳤거나 파일이 잘못 붙은 것이다.
    트리맵은 시가총액을 면적으로, 업종을 묶음으로 쓰므로 이 값이 흔들리면
    그림 자체가 달라진다.

    업종은 비어 있는 행이 있으므로 값이 들어 있는 행끼리만 견준다. 빈 값을
    채워 넣지는 않는다(→ AGENTS.md §9).
    """
    for column in STOCK_FACT_COLUMNS:
        rows = stock
        if column == "sector":
            rows = stock[stock["sector"] != ""]
        if rows.empty:
            continue
        counts = rows.groupby(
...
def stock_facts(stock: pd.DataFrame) -> pd.DataFrame:
    """종목마다 한 행인 표. 기준월·종목명·시가총액·업종을 담는다.
    ...
    return (
        ...
        .sort_values(["base_month", "market_cap"], ascending=[True, False])
