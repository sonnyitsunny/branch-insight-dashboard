...
def _transaction4_ai_frame() -> pd.DataFrame:
    """거래 탭 AI 요약 원본. 앞의 두 파일과 형식이 같고 글만 다르다."""
    rows = [
        {
            "CSMT_ORZ_NM": name,
...
def test_ai_summary_files_land_in_one_frame_split_by_topic(source_files):
    ...
    assert set(frame["topic"]) == {topic for topic, _ in AI_TOPIC_LABELS}
    assert len(frame) == len(BRANCHES) * len(AI_TOPIC_LABELS)
    for _, name in BRANCHES:
        for topic, suffix in AI_TOPIC_LABELS:
            assert _lines_of(data, topic, name) == _ai_summary(
                f"{name}{suffix}"
            )
    # '전체' 행은 지점 데이터에서 빠져 따로 담기고, 탭마다 하나씩이다.
    total = data.ai_summary_total
    assert len(total) == len(AI_TOPIC_LABELS)
...
def test_ai_summary_gets_its_branch_code_from_the_monthly_file(source_files):
    ...
    for code, name in BRANCHES:
        for topic, _ in AI_TOPIC_LABELS:
            assert rows.loc[(topic, name), "branch_id"] == code
...
def test_other_ai_summaries_are_checked_the_same_way(source_files, key, make):
    """다른 탭의 파일도 같은 검사를 지난다. 읽는 코드가 하나이기 때문이다."""
    frame = make()
    frame.loc[0, "TOPIC_SUMMARY"] = ""
    with pytest.raises(ValueError, match="비어 있는 행이"):
